...
program
  ...
  .option('-t, --template <template>', 'Template to use: default or plugin (default: default)', 'default')
  ...
  .option('-b, --beta', 'Init with beta template')
...
program
  ...
  .action(async () => {
    try {
      ...
      console.error('Error configuring CLI:', error.message);
      process.exit(1);
    }
  });
