 ...
 */
async function getAuthType({ proxyId, proxyConfig }: any = {}) {
 ...
 * @returns {Promise<Record<string, any>>}
 */
async function getUserInfo({ authHeader, hostname, additionalInfo, platform, apiKey, proxyId, proxyConfig, userEmail }: any = {}) {
  const cfg = proxyConfig ? proxyConfig : (await loadPlatformConfig(proxyId));
