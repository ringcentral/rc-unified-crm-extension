...
/**
 * @param {{ jwtToken: string, pluginId?: string }} params
...
/**
 * @param {{ user: any, callbackUri: string }} params
...
/**
 * @param {{ user: any, data: any, taskId?: string }} params
 * @returns {Promise<any>}
 */
async function uploadToGoogleDrive({ user, data, taskId }) {
    ...
    try {
        ...
        const fileName = `${(new Date() as any).toISOString("YYYY-MM-DD HH:mm:ss")}.mp3`;
