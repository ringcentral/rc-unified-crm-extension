...
describe('Bullhorn monthly report connector', () => {
    ...
    test('reads paged Salesforce records and joins duplicate Bullhorn ids by email', async () => {
        ...
        expect(report.csv).toContain('"100,200",,Shared,shared@example.com,Shared Co,Owner One,001ABC000000000001,RingCentral App Connect,,Active,,rc-account-1,success');
        expect(report.csv.split('\n').at(-1)).toBe(',,,,,,,,,,,rc-account-2,Not Found');
    });
    ...
    test('appends the RC account as Not Found when the Bullhorn profile lookup fails', async () => {
        ...
        expect(report.rowCount).toBe(1);
        expect(report.csv.split('\n').at(-1)).toBe(',,,,,,,,,,,rc-account-1,Not Found');
    ...
    test('appends token-invalid RC accounts after successful report rows', async () => {
        ...
        expect(lines.at(-1)).toBe(',,,,,,,,,,,rc-account-2,Token Invalid');
    });
